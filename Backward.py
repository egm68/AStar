...
def ComputePath(gridworld):
  ...
  while (s_goal.g_value > minInOpenListGMax().f_value):
        ...
        for i in range(4):
            ...
            if action != None and action.blocked == False:
                if action.search < counter:
                    action.g_value = float('inf')
                    action.search = counter
                if action.g_value > s_current.g_value + 1:
                    action.g_value = s_current.g_value + 1
...
def Backward(gridworld, startRow, startCol, endRow, endCol, pdf):
    global S
    global s_start
    global s_goal
    global openList
    ...
    counter = 0
    final_path.append(S[endRow][endCol])
...
pdf = matplotlib.backends.backend_pdf.PdfPages("Backward_Paths.pdf")
...
pdf = Backward(gridworld, startRow, startCol, endRow, endCol, pdf)
stop = timeit.default_timer()

print("Runtime of Backward was " + str(stop - start))
